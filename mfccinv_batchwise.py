...
readfile = 'test1.flac'
outfile = 'out.wav'
no_of_inter = 150  #No of phase iterations required while inverting
...
def make_mfcc(data1,rate,s,e):
    ...
    inverted_mel_sub = invert_pretty_spectrogram(np.transpose(mel_inverted_spectrogram), fft_size = fft_size,step_size = step_size, log = True, n_iter = no_of_inter)
    #print(len(inverted_mel_sub))
    return inverted_mel_sub



data1,rate = sf.read(readfile)
inverted_mel_audio = np.zeros(1)
no_of_samples =rate
end = 0
l = int(len(data1)/no_of_samples)
r = len(data1)%no_of_samples
for i in range (1,l+1,1):
    start = int((i-1)*rate)
    end = int(i*rate)
    inverted_mel_audio = np.concatenate((inverted_mel_audio,make_mfcc(data1,rate,start,end)))
    #print(len(inverted_mel_audio))
#print(inverted_mel_audio)
inverted_mel_audio = np.concatenate((inverted_mel_audio,make_mfcc(data1,rate,end,end+r)))
#print(len(inverted_mel_audio))
